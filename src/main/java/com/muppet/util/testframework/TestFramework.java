...
public class TestFramework {
	...
	public static void main(String[] args) {
		...
		config.setProperty(ApplicationConfig.AUTO_SCAN_PACKAGE_KEY,
				new String[]{"cn.bronzeware.data.test","cn.bronzeware.util.testframework"});
		...
		String path = String.format(FileUtil.convertFilePathOnOs("test/xml/%s.xml"), System.currentTimeMillis());
		...
		Map<String, Map<String, TestUnitMetaData>> oldMap = testFramework.getUnitMap("test/xml");
		storage.store(testFramework.getTestUnits());
		HtmlTestUnitStorage html = new HtmlTestUnitStorage(FileUtil.convertFilePathOnOs("test/html"));

...
/**
 * 单表查询语句生成，我们知道用户的单表操作，可以很方便的查询 不用每一次都指定 返回哪些数据，但是用户的查询条件是必须自己输入的
 * 
 * @author 于海强 2016-6-27 下午6:14:07
 */
public class SelectSqlGenerate implements SqlGenerate {
	 ...
	 */
	/*
	 * private static Map<String, String> factory = new HashMap<String,
	 * String>();
	 */

	/**
	 * 获取查询相关的sql语句 单表查询语句生成，我们知道用户的单表操作，可以很方便的查询 不用每一次都指定
	 * 返回哪些数据，但是用户的查询条件是必须自己输入的
	 * 
	 * @author 于海强 2016-6-27 下午6:14:59
	 */
	public String getSql(Sql sql) throws ParamCanNotBeNullException {
		 ...
		 */
		/*
		 * if(factory.containsKey(tableName)){ return factory.get(tableName); }
		 */
	...
	private boolean isAddWheres(String wheres) {
		...
		/*
		 * if(wheres.indexOf(" and ")==-1 &&wheres.indexOf(" or ")==-1
		 * &&wheres.indexOf('<')==-1 &&wheres.indexOf('>')==-1
		 * &&wheres.indexOf('=')==-1 &&wheres.indexOf("like")==-1
		 * &&wheres.indexOf("in")==-1){
		 * 
		 * return false; }
		 */
	...
	public static void main(String[] args) {
		...
		/*
		 * SqlGenerate sqlGenerate = new SelectSqlGenerate(); Sql sql = new
		 * Sql(); sql.setTableName("t_user"); sql.setWheres("id = 1"); String[]
		 * objectkeys = new String[2]; objectkeys[0] = "username"; objectkeys[1]
		 * = "password"; sql.setObjectkeys(objectkeys); try { String sqlString =
		 * sqlGenerate.getSql(sql); System.out.println(sqlString); } catch
		 * (ParamCanNotBeNullException e) { // e.printStackTrace(); }
		 */
	}
